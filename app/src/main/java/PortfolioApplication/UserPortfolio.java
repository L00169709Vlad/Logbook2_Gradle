...
public class UserPortfolio implements PortfolioSystem {
  ...
  public String listPortfolioAssetsByType(String assetType) {
    ...
    // checking if jsonobject has specific key and invoking Transactions.avgpurchaseprice() and
  ...
  private String validExchangeInput(String region) {
    ...
    exchInfo.httpRegRequest(region);
    ...
    while (!isCorrectNumber) {
      ...
      if (exchangeInput >= 0 && exchangeInput <= exchInfo.exchangeSymbol.size()) {
        List<String> inputExchangeList = exchInfo.getValidExchangeSym();
        --exchangeInput;
        ...
        System.out.println(
            "\n Invalid Exchange option number provided. Number must be between 1 and "
                + exchInfo.getValidExchangeSym().size());
      ...
      try {
        ...
        switch (option) {
          case 1:
            {
              ...
              System.out.println();
              // system stops and await user to decide to contoinue
          ...
          case 2:
            {
              // provide your functionality here
              double potrfolioValue = getPortfolioValue();
              double roundValue = Math.round(potrfolioValue * 100.0) / 100.0;
          ...
          case 4:
            {
              // Dislays the amount available for purchase of assets
              System.out.println(
